---
title: "KoboのEPUB表示崩れをCalibreで直す方法｜余白・フォント・目次の設定"
date: 2026-02-20
lastmod: 2026-06-22
draft: false
...
この手順は、自作EPUB、青空文庫、購入者が編集を許可されたDRMなしEPUBなど、自分が正当に扱えるファイルを対象にします。DRMの解除や、利用条件に反する変換は行わないでください。変換前の原本は必ず別フォルダへ保存します。
...
## 動作確認用の端末を選ぶならKobo Clara BW

文字中心のDRMなしEPUBを確認する用途では、現行6インチ白黒モデルのKobo Clara BWが比較しやすい候補です。楽天の公式ラインナップでは16GB、防水、EPUB対応が案内されています。カラー画像を重視する場合はClara Colour、見開きや物理ボタンを重視する場合はLibra Colourも比較してください。

すでにKobo端末を持っている場合は買い替える必要はありません。端末固有の問題か、EPUBファイル側の問題かを切り分けられない場合だけ現行モデルの仕様を確認してください。

{{< affbtn url="https://books.rakuten.co.jp/rb/17822116/" text="Kobo Clara BWの仕様と在庫を確認" store="rakuten" slot="kobo-epub-clara-bw" >}}
